...
for restart in range(num_restarts):
    ...
    while True:
        ...
        if cnt == 0:
            ...
            T_new_init = t_final
            good_inds_random = jnp.where((p_empirical_random[sampled_inds_random] > 0) & (p_empirical[sampled_inds_random] < 1))[0]
...
cell = 0
p = 0
electrical_path = 'test'
vis_datapath = 'test'
savemat('active_learning-01-1elec-{}-n{}-p{}-r{}.mat'.format(dataset, str(cell), str(p), str(args.restarts)),
        {'dataset': dataset,
            'cell': cell,
            'pattern': p,
