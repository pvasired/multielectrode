...
def negLL_hotspot_erf(params, *args):
    """
    Compute the negative log likelihood for a logistic regression
    binary classification task assuming the hotpot model of activation.
    Uses an error function to model the probability of activation.
    ...
    *args (tuple): X (np.ndarray) output of convertToBinaryClassifier,
    ...
    # Get predicted probability of spike using current parameters
    response_mat = 0.5 + 0.5*erf(X @ w.T)
...
# Numpy version of activation_probs()
def sigmoidND_nonlinear_erf(X, w):
    """
    N-dimensional nonlinear sigmoid computed according to multi-
    hotspot model.
...
def fit_surface_erf(X_expt, probs, T, w_inits_, bootstrapping=None, X_all=None,
    ...
    if bootstrapping is not None:
        ...
        for k in range(bootstrapping):
            ...
            # Now begin the McFadden pseudo-R2 early stopping loop
            if reg_method == 'MAP':
                last_opt = get_w_erf(w_inits[0], Xdata, ydata, zero_prob=zero_prob, 
                                method=method, jac=jac, reg_method=reg_method, 
                                reg=(reg[0], reg[1][0][0], reg[1][0][1]),
                                verbose=opt_verbose, slope_bound=slope_bound)
            else:
            ...
            for i in range(1, len(w_inits)):
                # Refit with next number of sites
                if reg_method == 'MAP':
                    new_opt = get_w_erf(w_inits[i], Xdata, ydata, zero_prob=zero_prob,
                                    method=method,
                                    jac=jac,
                                    reg_method=reg_method,
                                    reg=(reg[0], reg[1][i][0], reg[1][i][1]),
            ...
            probs_pred = sigmoidND_nonlinear_erf(sm.add_constant(X_all, has_constant='add'),
                                                last_opt[0])
            y_bootstrapped[k] = probs_pred
        ...
        # Now begin the McFadden pseudo-R2 early stopping loop
        if reg_method == 'MAP':
            last_opt = get_w_erf(w_inits[0], sm.add_constant(X_all, has_constant='add'), 
                            y_bootstrapped_avg, zero_prob=zero_prob, 
                            method=method, jac=jac, reg_method=reg_method, 
                            reg=(reg[0], reg[1][0][0], reg[1][0][1]),
                            verbose=opt_verbose, slope_bound=slope_bound)
        ...
        for i in range(1, len(w_inits)):
            # Refit with next number of sites
            if reg_method == 'MAP':
                new_opt = get_w_erf(w_inits[i], sm.add_constant(X_all, has_constant='add'),
                                y_bootstrapped_avg, zero_prob=zero_prob,
                                method=method,
                                jac=jac,
                                reg_method=reg_method,
        ...
        # Now begin the McFadden pseudo-R2 early stopping loop
        if reg_method == 'MAP':
            last_opt = get_w_erf(w_inits[0], X_bin, y_bin, zero_prob=zero_prob, 
                            method=method, jac=jac, reg_method=reg_method, 
                            reg=(reg[0], reg[1][0][0], reg[1][0][1]),
                            verbose=opt_verbose, slope_bound=slope_bound)
        else:
        ...
        for i in range(1, len(w_inits)):
            # Refit with next number of sites
            if reg_method == 'MAP':
                new_opt = get_w_erf(w_inits[i], X_bin, y_bin, zero_prob=zero_prob,
                                method=method,
                                jac=jac,
                                reg_method=reg_method,
                                reg=(reg[0], reg[1][i][0], reg[1][i][1]),
...
def get_w_erf(w_init, X, y, zero_prob=0.01, method='L-BFGS-B', jac=None,
    ...
    z = 1 - 2*(1 - zero_prob)**(1/len(w_init))

    # Set up bounds for constrained optimization
    bounds = []
    for j in range(len(w_init)):
        bounds += [(bias_bound, erfinv(z))]
        for i in range(X.shape[-1] - 1):
            bounds += [(-slope_bound, slope_bound)]

    ybar = np.mean(y)
    beta_null = erfinv(2*ybar - 1)
    null_weights = np.concatenate((np.array([beta_null]), 
                                   np.zeros(X.shape[-1]-1)))
    nll_null = negLL_hotspot_erf(null_weights, X, y, False, reg_method, reg[0])

    # Optimize the weight vector with MLE
    opt = minimize(negLL_hotspot_erf, x0=w_init.ravel(), bounds=bounds,
                       args=(X, y, verbose, reg_method, reg[0]), method=method,
                        jac=jac, options=options)
